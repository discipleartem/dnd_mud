...
- Старт обычно с **1 уровня**, 0 опыта; режим **Лёгкая** (`easy`) — с **3 уровня** и минимальным опытом для этого уровня (900 XP).
- Повышение уровня — по таблице опыта PHB (уровни **1–10** в MUD); см. `core/progression.py`.
- Опыт **накапливается** (`grant_experience`); после левелапа избыток над порогом **не обрезается** (например, 1500 XP на 3 уровне — нормально).
...
На **каждом** уровне (включая 1-й): бросок кости хитов класса + модификатор Телосложения. Минимум **1 HP** от части «кость + CON» (`max(1, …)`); расовые и чертовые бонусы добавляются сверху.
