...
- **Подкласс** (архетип) обычно с 3 уровня.
...
| Заметки | `features` и `subclasses` в YAML — справочные; runtime не применяет умения |
...
- Подклассы, прогрессия уровней, ячейки заклинаний
