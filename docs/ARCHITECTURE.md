...
| `core/slug.py` | `make_save_slug()` — slug сохранения персонажа |
| `core/stats.py` | Генерация/валидация характеристик, `apply_racial_bonuses_to_stats` |
...
| `core/grants.py` | Нормализация `grants[]` из YAML (legacy `features`/`mechanics`) |
...
**Сценарий «Создать персонажа»:** сложность → имя → раса → подраса → характеристики → предыстория → языки → класс → подкласс → навыки → сохранение в `saves/characters/{save_slug}.json`.

- Оркестрация: `ui/menus/character_flow.py`, `ui/menus/stats/stats_flow.py`
