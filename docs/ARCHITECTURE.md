...
| `saves/characters/*.json` | Персонажи (по одному файлу) | JSON | `character.py` |
...
Фильтрация приключений: `core/difficulty.py` + `_select_adventure()` в `ui/menus.py`. В `adventures.yaml` ограничения (`allowed_game_difficulties`, `hardcore_only`) пока не заданы — все приключения доступны в обоих режимах.  
