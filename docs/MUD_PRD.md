...
- Главное меню (6 пунктов + Выход: Новая игра, Загрузить игру, Персонажи, Настройки, Languages, Модификации)
...
- Flow «Создать персонажа» (сложность → имя → раса → подраса → характеристики → предыстория → языки → класс → подкласс → владения → навыки → снаряжение → сохранение)
- Flow «Загрузить игру» (список сессий `saves/sessions/` → resume с `current_node_id`)
- Hub «Персонажи» (список, создание, левелап, удаление)
- Меню «Модификации» (`ui/menus/mods_menu.py`)
...
- Полноценный движок приключений (комнаты, проверки, бой) — Phase 2
- Расширение mod-system (`requires`/`conflicts`, overlay `delete`/`replace_entity`) — см. DATA_SCHEMA §Phase 2+
...
|-------------|--------|---------|
| Проверки характеристик, навыки, преимущество/помеха в engine | Запланировано | Phase 2 engine; см. `07-ability-scores.md` |
...
| Прогрессия (XP, уровни, HP при левелапе) | Частично | XP 1–10; левелап через UI (`level_up`); HP по режиму — §3.2.1; умения в бою — Phase 2 |
| Игровой движок приключений | Частично | Минимальный `run_scenario()`; комнаты, проверки, бой — Phase 2 |
| Загрузка игры | Реализовано | `saves/sessions/`, `ui/menus/load_game.py`, resume сценария |
...
Нумерованный список пунктов. По умолчанию от пользователя ожидается ввод **integer** из диапазона 0–6. Пункт "0" — выход из игры/приложения.

1. Новая игра
2. Загрузить игру
3. Персонажи (hub: создание, левелап, удаление)
4. Настройки
5. Languages
6. Модификации
0. Выход

**Текущее состояние:** все пункты реализованы (`main.py` → `ui/menus/`).
...
Запускается из hub «Персонажи» (главное меню §3.2). Пользователь последовательно проходит через экраны:

**Порядок шагов:** сложность → имя → раса → подраса → генерация характеристик → **предыстория** → **языки** → класс → подкласс (если предусмотрен режимом) → **владения** → **навыки** → (компетентность?) → **снаряжение** → сохранение.
...
- После навыков (и expertise при необходимости) — §3.4.13 (снаряжение), затем сохранение.
...
├── adventures/                      # сценарии приключений (YAML-графы)
│   ├── tutorial.yaml
│   └── lost_mine.yaml
├── mods/                            # контент-паки (manifest + overlay)
│   └── dragonborn_pack/
...
### 6.2. UI
- `ui/menus/` – пакет flows и экранов (`show_welcome_screen`, `show_main_menu`, `show_settings`, …).
...
Пример главного меню (английская локаль):
