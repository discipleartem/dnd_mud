...
|-------------|--------|---------|
| Проверки характеристик, навыки, спасброски, преимущество/помеха | Запланировано | Сейчас: `ability_modifier()` и генерация stats; см. `07-ability-scores.md` |
| Бонус мастерства | Запланировано | Не хранится в `Character`, не считается по уровню |
| Бой (инициатива, атаки, урон, смерть, укрытие) | Запланировано | `current_hp` в модели — задел; см. `09-combat.md` |
| Заклинания и ячейки | Запланировано | См. `10-spells.md` |
| Снаряжение, КД, оружие | Запланировано | YAML в `database/_future/`; см. `05-equipment.md` |
| Предыстории | Частично | Выбор в flow; см. `04-backgrounds.md` |
| Мультикласс | **Запрещено** | Один класс на персонажа (`class_name`); см. `06-multiclass-feats.md` |
| Черты (feats) | Запланировано | Variant human: черта не выбирается; см. `06-multiclass-feats.md` |
...
Целевая реализация: `game_engine.py`, загрузка сценариев из `adventures/*.yaml`, справочники Phase 2 из `database/_future/`. Режим HardCore — полные правила в engine; Normal — допустимые упрощения в YAML-приключениях (§3.2.1).
...
**Порядок шагов:** сложность → имя → раса → подраса → генерация характеристик → **предыстория** → **языки** → класс → подкласс (если предусмотрен режимом) → **навыки** → (компетентность?) → сохранение.
...
**Особенности рас (features).** На экране выбора показываются название и описание из YAML. **Игровая механика** особенностей (тёмное зрение, сопротивления, владения навыками и т.д.) **на данном этапе не реализована** — будет подключена вместе с game engine (Phase 2). Исключение: выборные бонусы к характеристикам (`type: ability_bonus`, напр. variant human) — реализованы в flow генерации stats (§3.4.6).
...
- Инструменты, снаряжение и механика feature — данные в YAML, UI/engine v1 не реализованы.
...
- Тип ввода: **integer** по нумерованному списку.
- После выбора класса: если `subclass_offered_at_creation(difficulty, class_id)` — переход к §3.4.10; иначе §3.4.11 (навыки) или сохранение.

**Один класс на персонажа.** Мультиклассирование **запрещено** на текущем этапе: при создании выбирается один класс (`class_name`), при повышении уровня класс не меняется. См. [`06-multiclass-feats.md`](rules/06-multiclass-feats.md).
...
- После выбора (или пропуска на hardcore) — §3.4.11 (навыки) или сохранение.
...
JSON персонажа включает `"languages": [...]`, `"background": "<id>"`, `"skills": [...]`.
...
│   │   ├── core/                    # abilities, skills, languages, …
│   │   ├── equipment/               # armor, weapon, tools, …
│   │   ├── progression/             # backgrounds, feats
...
| 7 | В `database/_future/core/abilities.yaml` CON как «Выносливость» | При активации Phase 2 сверить с PHB | `07-ability-scores.md` | Phase 2 |
