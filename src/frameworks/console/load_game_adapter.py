...
from src.dto.save_game_dto import SaveGameDTO, SaveSlotDTO, CharacterPreviewDTO
...
class LoadGameMenuAdapter:
    ...
    def display_load_menu(self, slots: List[SaveSlotDTO]) -> None:
        ...
        for i in range(0, len(slots), 5):
            ...
            # Заголовки колонок
            print(f"{'Слот':<6} {'Персонаж':<20} {'Уровень':<8} {'Класс':<15} {'Время':<12}")
            print("-" * 65)
    ...
    def display_save_preview(self, save_info: SaveGameDTO, preview: CharacterPreviewDTO) -> None:
        ...
        print(f"  Время игры: {save_info.playtime_minutes // 60}ч {save_info.playtime_minutes % 60}м")
    ...
    def confirm_load(self, save_info: SaveGameDTO) -> bool:
        ...
        print(f"  {save_info.character_name} (уровень {save_info.character_level} {save_info.character_class})")
        print(f"  Слот {save_info.slot_number} от {save_info.save_time}")
        print()
        
        while True:
            try:
                choice = input("Загрузить это сохранение? (да/нет): ").strip().lower()
    ...
    def confirm_delete(self, save_info: SaveGameDTO) -> bool:
        ...
        print(f"Вы собираетесь удалить сохранение:")
        print(f"  {save_info.character_name} (уровень {save_info.character_level} {save_info.character_class})")
        print(f"  Слот {save_info.slot_number} от {save_info.save_time}")
        ...
        while True:
            try:
                choice = input("Удалить это сохранение? (да/нет): ").strip().lower()
