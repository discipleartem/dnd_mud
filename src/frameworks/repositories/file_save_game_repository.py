...
class FileSaveGameRepository(SaveGameRepository):
    ...
    def _create_metadata_file(self) -> None:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка создания файла метаданных: {e}")
    ...
    def _load_metadata(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка чтения файла метаданных: {e}")
        except Exception as e:
            raise RepositoryError(f"Ошибка загрузки метаданных: {e}")
    ...
    def _save_metadata(self, metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка сохранения метаданных: {e}")
    ...
    def save(self, save_game: SaveGame, game_data: Dict[str, Any]) -> SaveGame:
        ...
        try:
            ...
            existing_saves = [
                s for s in metadata["saves"] 
                if s["slot_number"] == save_game.slot_number and s["save_id"] != save_game.save_id
            ]
            ...
            if existing_saves:
                ...
                metadata = self._load_metadata()  # Перезагружаем после удаления
            ...
            if isinstance(e, RepositoryError):
                raise
            raise RepositoryError(f"Ошибка сохранения игры: {e}")
    ...
    def load(self, save_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка чтения файла сохранения: {e}")
        except Exception as e:
            if isinstance(e, RepositoryError):
                raise
            raise RepositoryError(f"Ошибка загрузки игры: {e}")
    ...
    def find_by_id(self, save_id: str) -> Optional[SaveGame]:
        ...
        try:
            ...
            if isinstance(e, RepositoryError):
                raise
            raise RepositoryError(f"Ошибка поиска сохранения: {e}")
    ...
    def find_all(self) -> List[SaveGame]:
        ...
        try:
            ...
            if isinstance(e, RepositoryError):
                raise
            raise RepositoryError(f"Ошибка получения сохранений: {e}")
    ...
    def delete(self, save_id: str) -> bool:
        ...
        try:
            ...
            if isinstance(e, RepositoryError):
                raise
            raise RepositoryError(f"Ошибка удаления сохранения: {e}")
