...
class SettingsController:
    ...
    def __init__(self, settings_use_case: SettingsUseCase) -> None:
        ...
        self._current_settings: SettingsStateDTO | None = None
    ...
    def handle_request(
        self, request: SettingsControllerRequest
    ) -> SettingsControllerResponse:
        ...
        try:
            ...
            return SettingsControllerResponse(
                success=False, message=f"Ошибка обработки запроса: {str(e)}"
            )

    def get_current_settings(self) -> SettingsStateDTO | None:
    ...
    def _handle_get_request(
        ...
        if request.key:
            ...
            if setting:
                setting_dto = SettingDTO.from_setting(setting)
                return SettingsControllerResponse(
                    success=True,
            ...
            setting_dtos = [SettingDTO.from_setting(s) for s in settings]
    ...
    def _handle_set_request(
        ...
        if not request.key or request.value is None:
            return SettingsControllerResponse(
                success=False, message="Не указан ключ или значение настройки"
            )

        success = self._settings_use_case.set_setting(
        ...
        if success:
            ...
            setting = self._settings_use_case.get_setting(request.key)
            setting_dto = SettingDTO.from_setting(setting) if setting else None
            ...
            return SettingsControllerResponse(
                success=False, message="Не удалось установить настройку"
            )
    ...
    def _handle_reset_request(self) -> SettingsControllerResponse:
        ...
        if success:
            ...
            return SettingsControllerResponse(
                success=False, message="Не удалось сбросить настройки"
            )
    ...
    def _handle_export_request(self) -> SettingsControllerResponse:
        ...
        if settings_data is not None:
            ...
            return SettingsControllerResponse(
                success=False, message="Не удалось экспортировать настройки"
            )

    def _handle_import_request(
        self, request: SettingsControllerRequest
    ) -> SettingsControllerResponse:
        ...
        if not request.settings_data:
            return SettingsControllerResponse(
                success=False, message="Не указаны данные для импорта"
            )

        success = self._settings_use_case.import_settings(
        ...
        if success:
            ...
            return SettingsControllerResponse(
                success=False, message="Не удалось импортировать настройки"
            )
