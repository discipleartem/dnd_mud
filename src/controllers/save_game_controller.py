...
class SaveGameController:
    ...
    def handle_request(self, request: SaveGameRequest) -> SaveGameResponse:
        ...
        try:
            ...
            return SaveGameResponse(
                success=False,
                message=f"Ошибка репозитория: {str(e)}"
            )
    ...
    def _handle_save(self, request: SaveGameRequest) -> SaveGameResponse:
        ...
        try:
            ...
            save_dto = SaveGameDTO.from_entity(save_game)
    ...
    def _handle_load(self, request: SaveGameRequest) -> SaveGameResponse:
        ...
        if not request.save_id:
            return SaveGameResponse(
                success=False,
                message="Отсутствует ID сохранения"
            )
        ...
        try:
            ...
            save_dto = SaveGameDTO.from_entity(save_game) if save_game else None
    ...
    def get_save_slots_info(self) -> List[SaveSlotDTO]:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка получения информации о слотах: {e}")
    
    def get_character_preview(self, save_id: str) -> Optional[CharacterPreviewDTO]:
        ...
        try:
            ...
            raise RepositoryError(f"Ошибка получения предпросмотра: {e}")
    ...
    def quick_save(
        ...
        try:
            ...
            return SaveGameResponse(
                success=False,
                message=f"Ошибка быстрого сохранения: {str(e)}"
            )
