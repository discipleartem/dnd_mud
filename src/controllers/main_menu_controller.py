...
from src.dto.welcome_dto import WelcomeControllerResponse
...
class MainMenuController:
    ...
    def __init__(self, menu_use_case: MenuNavigationUseCase) -> None:
        ...
        self._current_menu_state: Optional[MenuStateDTO] = None
    ...
    def create_main_menu(self, language: str = "ru") -> MenuControllerResponse:
        ...
        try:
            ...
            menu_state = self._menu_use_case.create_menu(
                title=self._get_menu_title(language),
                items=menu_items
            )
    ...
    def handle_navigation(self, request: MenuControllerRequest) -> MenuControllerResponse:
    ...
    def get_current_menu_state(self) -> Optional[MenuStateDTO]:
    ...
    def _handle_navigation_request(self, request: MenuControllerRequest) -> MenuControllerResponse:
        ...
        selectable_items = [
            item for item in self._current_menu_state.items 
    ...
    def _handle_selection_request(self, request: MenuControllerRequest) -> MenuControllerResponse:
        ...
        if request.selection.isdigit():
            ...
            if 1 <= selection_index <= len(selectable_items):
                ...
                for item in self._current_menu_state.items:
                    if item.action_type == MenuActionType.NAVIGATION and item.target_menu == "settings":
                        selected_item = item
                        break
        else:
            # Проверяем выбор по горячей клавише
            for item in selectable_items:
                if item.hotkey and item.hotkey.lower() == request.selection.lower():
                    selected_item = item
                    break
        
        # Если пункт выбран, выполняем действие
        if selected_item:
            if selected_item.action_type == MenuActionType.ACTION and selected_item.action:
