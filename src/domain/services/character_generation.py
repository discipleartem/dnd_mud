...
from ..entities.race_factory import RaceFactory
...
class GenerationConfig:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], method_type: GenerationMethod) -> 'GenerationConfig':
...
class AttributeGenerator:
    ...
    def _load_configs(cls) -> None:
        ...
        try:
            ...
            if 'standard_array' in generation_data:
                cls._generation_configs[GenerationMethod.STANDARD_ARRAY] = StandardArrayConfig.from_dict(
                    generation_data['standard_array']
                )
            
            # 4d6 drop lowest
            if 'four_d6_drop_lowest' in generation_data:
                cls._generation_configs[GenerationMethod.FOUR_D6_DROP_LOWEST] = FourD6Config.from_dict(
                    generation_data['four_d6_drop_lowest']
                )
            
            # Покупка очков
            if 'point_buy' in generation_data:
                cls._generation_configs[GenerationMethod.POINT_BUY] = PointBuyConfig.from_dict(
                    generation_data['point_buy']
                )
                
        except FileNotFoundError:
            # Fallback конфигурации
            cls._generation_configs[GenerationMethod.STANDARD_ARRAY] = StandardArrayConfig(
                "Стандартный набор", "Стандартный набор (15, 14, 13, 12, 10, 8)", 
                GenerationMethod.STANDARD_ARRAY, [15, 14, 13, 12, 10, 8]
            )
            cls._generation_configs[GenerationMethod.FOUR_D6_DROP_LOWEST] = FourD6Config(
                "4d6", "Бросок 4d6, отбрасываем наименьший", 
                GenerationMethod.FOUR_D6_DROP_LOWEST, 4, 1
            )
            cls._generation_configs[GenerationMethod.POINT_BUY] = PointBuyConfig(
                "Покупка очков", "Распределение очков по выбору (27 очков)", 
                GenerationMethod.POINT_BUY, 27, 8, 15
            )
    ...
    def validate_point_buy_attributes(cls, attributes: Dict[str, int]) -> bool:
        ...
        total_cost = sum(cls.get_point_buy_costs().get(value, 999) for value in attributes.values())
...
class CharacterBuilder:
    ...
    def set_attributes_point_buy(self, attributes: Dict[str, int]) -> 'CharacterBuilder':
    ...
    def build(self) -> Character:
        ...
        race = RaceFactory.create_race(self._race_name)
...
class CharacterFactory:
    ...
    @staticmethod
    def create_standard_character(name: str = "Безымянный", level: int = 1) -> Character:
    ...
    @staticmethod
    def create_optimized_character(name: str = "Безымянный", level: int = 1, 
                                  race_name: str = "human", class_name: str = "fighter") -> Character:
...
if __name__ == "__main__":
    ...
    print(f"Сила: {random_char.strength.value} (+{random_char.get_ability_modifier(random_char.strength.value)})")
    print(f"Ловкость: {random_char.dexterity.value} (+{random_char.get_ability_modifier(random_char.dexterity.value)})")
    print(f"Телосложение: {random_char.constitution.value} (+{random_char.get_ability_modifier(random_char.constitution.value)})")
