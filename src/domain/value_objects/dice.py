...
class Dice:
    ...
    def roll_with_modifier(
        self, modifier_type: RollModifierType, modifier: int = 0
    ) -> DiceResult:
        ...
        rolls = [
            random.randint(1, self._config.sides) for _ in range(modifier_config.rolls)
        ]
...
class RollModifierConfig:
    ...
    def _load_config(cls, modifier_name: str) -> Optional["RollModifierConfig"]:
        ...
        fallback_configs = {
            ...
            "triple_advantage": cls(
                "triple_advantage", "Тройное преимущество", "best_of", 3
            ),
            "triple_disadvantage": cls(
                "triple_disadvantage", "Тройная помеха", "worst_of", 3
            ),
        }
...
class DiceRoll:
    ...
    @staticmethod
    def roll_multiple(
        dice_type: str, count: int, modifier: int = 0
    ) -> List[DiceResult]:
