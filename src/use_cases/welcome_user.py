...
class WelcomeUserUseCase:
    ...
    def _validate_content(self, translations: dict) -> None:
        ...
        if not translations.get("subtitle") or not translations["subtitle"].strip():
            raise ValueError("Подзаголовок не может быть пустым")
        
        if not translations.get("description") or not translations["description"].strip():
            raise ValueError("Описание не может быть пустым")
        
        if not translations.get("press_enter") or not translations["press_enter"].strip():
    ...
    def _get_translations(self, language_code: str) -> dict:
        ...
        translations = {
            ...
            "subtitle": self._translation_service.get_translation(
                language_code, "welcome_subtitle", "Текстовая многопользовательская ролевая игра"
            ),
            "description": self._translation_service.get_translation(
                language_code, "welcome_description", 
            ...
            "press_enter": self._translation_service.get_translation(
                language_code, "press_enter", "Нажмите Enter для продолжения..."
            )
        }
        
        return translations
    
    def _get_fallback_response(self, request: WelcomeRequest) -> WelcomeResponse:
        ...
        if request.show_ascii_art:
            try:
                ascii_art = self._get_simple_ascii_art()
            except:
                ascii_art = None
