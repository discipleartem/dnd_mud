...
class MenuItem(BaseValidatable):
    ...
    def validate(self) -> None:
        ...
        if len(self.title) > 50:
            raise ValueError(
                "Заголовок пункта меню слишком длинный (макс 50 символов)"
            )

        if (
            self.action_type == MenuActionType.NAVIGATION
            and not self.target_menu
        ):
            raise ValueError(
                "Для пункта навигации необходимо указать target_menu"
            )
...
class MenuState(BaseValidatable):
    ...
    def validate(self) -> None:
        ...
        if not visible_items:
            raise ValueError(
                "Меню должно содержать хотя бы один видимый пункт"
            )
    ...
    def get_current_item(self) -> MenuItem | None:
    ...
    def select_by_hotkey(self, hotkey: str) -> MenuItem | None:
