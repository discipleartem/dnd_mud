...
class NameInputAdapter:
    ...
    def prompt_for_name(self) -> Optional[Character]:
        ...
        while True:
            try:
                ...
                print(f"Создан: {character.created_at.strftime('%d.%m.%Y %H:%M')}")
                print()
    ...
    def show_success_message(self, character: Character) -> None:
        ...
        print(f"Персонаж сохранен в файл data/characters.json")
        print(f"ID: {character.id}")
        print(f"Создан: {character.created_at.strftime('%d.%m.%Y %H:%M')}")
    ...
    def show_cancellation_message(self) -> None:
        ...
        print("\nСоздание персонажа отменено.")
        print("Вы можете вернуться к этому позже через меню 'Создать персонажа'.")
        print()
