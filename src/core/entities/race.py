...
class Race:
    ...
    def __post_init__(self) -> None:
        ...
        if self.name:
            info = localization.get_race_info(self.name)
            ...
            if not self.bonuses:
                self.bonuses = info.get('bonuses', {})
            # Загружаем альтернативные особенности
            if not self.alternative_features:
                self.alternative_features = info.get('alternative_features', {})
    
    @property
    def localized_name(self) -> str:
        """Возвращает локализованное название."""
        return localization.get_race_name(self.name)
    ...
    def apply_alternative_bonuses(self, attributes: Dict[str, int], choices: Dict[str, Any]) -> Dict[str, int]:
