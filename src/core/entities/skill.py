...
class Skill:
    ...
    custom_bonus: int = field(default=0)  # Дополнительные бонусы от предметов/заклинаний
    ...
    def __post_init__(self) -> None:
        ...
        if self.proficiency_bonus < 0:
            raise ValueError(f"Бонус мастерства не может быть отрицательным: {self.proficiency_bonus}")
        
        if self.expertise_bonus < 0:
            raise ValueError(f"Бонус экспертизы не может быть отрицательным: {self.expertise_bonus}")
    ...
    def localized_name(self) -> str:
        ...
        return localization.get_skill_name(self.name)
    
    @property
    def localized_description(self) -> str:
        """Возвращает локализованное описание."""
        return localization.get_skill_description(self.name)
    ...
    def calculate_total_bonus(self, character_attributes: dict, proficiency_bonus: int, penalties: dict = None) -> int:
    ...
    def __repr__(self) -> str:
        ...
        return (f"Skill(name='{self.name}', proficiency_bonus={self.proficiency_bonus}, "
