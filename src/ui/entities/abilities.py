...
class AbilityScores:
    ...
    def set_base_score(self, ability: Ability, value: int) -> None:
        ...
        if 1 <= value <= 20:
            ...
            raise ValueError("Значение характеристики должно быть от 1 до 20")
...
class PointBuySystem:
    ...
    def buy_score(self, old_value: int, new_value: int) -> bool:
        ...
        if new_value not in PointBuyCosts.get_valid_values():
            return False
...
class RandomGeneration:
    ...
    def assign_randomly(scores: list[int]) -> dict[Ability, int]:
        """Случайно распределить значения по характеристикам
        (hardcore режим)."""
        abilities = list(Ability)
        random.shuffle(abilities)
        return dict(zip(abilities, scores, strict=False))
