...
from src.ui.entities.abilities import (
    Ability, AbilityScores, StandardArray, PointBuySystem, 
    RandomGeneration, PointBuyCosts
)
...
def display_racial_bonuses(race: Race, subrace: Optional[SubRace] = None) -> None:
...
def display_abilities_with_bonuses(ability_scores: AbilityScores) -> None:
    ...
    print("-" * 60)
    print(f"{'Характеристика':<15} {'Базовое':<10} {'Бонус':<10} {'Итого':<10} {'Модификатор':<12}")
    print("-" * 60)
    ...
    for ability in Ability:
        ...
        print(f"{ability.get_localized_name():<15} {base:<10} {bonus_str:<10} {total:<10} {modifier_str:<12}")
...
def generate_standard_array(ability_scores: AbilityScores) -> None:
    ...
    print(f"📊 {t('ability_generation.standard.values')}: {', '.join(map(str, values))}")
    print()
    ...
    for ability in abilities:
        while True:
            ...
            print(f"{t('ability_generation.standard.remaining_values')}: {', '.join(map(str, remaining_values))}")
            
            try:
                choice = input(f"Выберите значение: ").strip()
                ...
                try:
                    ...
                    if value in remaining_values:
                        ...
                        print(f"❌ Значение {value} отсутствует в списке доступных")
                except ValueError:
                    print(t('ability_generation.error_invalid'))
            except ValueError:
                print(t('ability_generation.error_invalid'))
...
def generate_point_buy(ability_scores: AbilityScores) -> None:
    ...
    print(f"💰 {t('ability_generation.point_buy.total_points')}: {point_buy.POINTS_TOTAL}")
    print()
    ...
    # Устанавливаем начальные значения (1 для всех характеристик - минимальально допустимое)
    for ability in Ability:
    ...
    for ability in abilities:
        _distribute_single_ability(ability, ability_scores, assigned_values, remaining_points, valid_values)
        # Обновляем remaining_points после каждого распределения
        remaining_points = 27 - sum(PointBuyCosts.get_cost(assigned_values[a]) for a in abilities if assigned_values.get(a, 1) != 1)
    
    # Устанавливаем 8 для нераспределенных характеристик (тех что остались со значением 1)
    for ability in abilities:
        if assigned_values.get(ability, 1) == 1:
            ability_scores.set_base_score(ability, 8)
            print(f"⚪ {ability.get_localized_name()}: установлено значение 8 (по умолчанию)")
    
    # Финальное подтверждение
    print(f"\n{t('ability_generation.final.confirmation')}")
    print("=" * 40)
    display_abilities_with_bonuses(ability_scores)
    print(f"💰 {t('ability_generation.point_buy.remaining_points')}: {remaining_points}")
    
    # Проверяем, можно ли подтверждать выбор
    unassigned_abilities = [a for a in abilities if assigned_values.get(a, 1) == 1]
    ...
    if not can_confirm:
        if unassigned_abilities:
            ...
            unassigned_names = [a.get_localized_name() for a in unassigned_abilities]
    ...
    while True:
        ...
        if choice == '1':
            if can_confirm:
                ...
                print("❌ Невозможно подтвердить выбор! Сначала распределите все характеристики и потратьте все очки.")
        elif choice == '2':
...
def _distribute_single_ability(ability: Ability, ability_scores: AbilityScores, assigned_values: dict, remaining_points: int, valid_values: List[int]) -> None:
    ...
    while True:
        ...
        print(f"💰 {t('ability_generation.point_buy.remaining_points')}: {remaining_points}")
        print(f"📊 {t('ability_generation.point_buy.available_values')}: {', '.join(map(str, available_values))}")
        
        try:
            choice = input(f"Выберите значение (0 для пропуска): ").strip()
            ...
            try:
                ...
                if value in available_values:
                    ...
                    assigned_values[ability] = value
                    ability_scores.set_base_score(ability, value)
                    print(f"✅ {ability.get_localized_name()}: {value} (стоимость: {cost} очков)")
                    break
                else:
                    cost = PointBuyCosts.get_cost(value) if value in valid_values else 0
                    if cost > remaining_points:
                        print(f"❌ Значение {value} стоит {cost} очков, у вас осталось только {remaining_points}")
                    else:
                        print(f"❌ Значение {value} недоступно. Доступные значения: {', '.join(map(str, available_values))}")
            except ValueError:
                print(t('ability_generation.error_invalid'))
        except ValueError:
            print(t('ability_generation.error_invalid'))
...
def generate_random_scores(ability_scores: AbilityScores, hardcore_mode: bool = False) -> None:
    ...
    print(f"🎲 {t('ability_generation.random.rolled_values')}: {', '.join(map(str, scores))}")
    print()
    ...
    if hardcore_mode:
        ...
        for ability in abilities:
            while True:
                ...
                print(f"{t('ability_generation.random.remaining_values')}: {', '.join(map(str, remaining_scores))}")
...
def handle_variant_human_ability_choice(ability_scores: AbilityScores) -> None:
    ...
    print("=" * 50)
    print(f"Выберите 2 характеристики для увеличения на +1")
    print()
    ...
    while choices_made < max_choices:
        ...
        for i, ability in enumerate(abilities, 1):
            ...
            modifier = ability_scores.get_modifier(ability)
            modifier_str = f"+{modifier}" if modifier >= 0 else str(modifier)
            print(f"{i}. {ability.get_localized_name()}: {current_total} (мод. {modifier_str})")
        
        try:
            choice = input(f"\nВыберите характеристику ({choices_made + 1}/{max_choices}): ").strip()
            ...
            if choice.isdigit():
                ...
                if 0 <= index < len(abilities):
                    ...
                    # Применяем бонус
                    current_bonus = ability_scores.racial_bonuses.get(selected_ability, 0)
                    ability_scores.racial_bonuses[selected_ability] = current_bonus + 1
                    
                    print(f"✅ {selected_ability.get_localized_name()} увеличена на +1")
                    choices_made += 1
...
def generate_ability_scores(race: Race, subrace: Optional[SubRace] = None, hardcore_mode: bool = False) -> AbilityScores:
...
def _has_ability_choice_feature(subrace: SubRace) -> bool:
    ...
    for feature in subrace.features:
        ...
        if (mechanics.get('type') == 'ability_bonus' and 
            mechanics.get('target') == 'choice' and 
            mechanics.get('choice') is True):
            return True
    return False
