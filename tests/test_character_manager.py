...
from src.adapters.repositories.character_repository import (
    CharacterSaveData, CharacterRepository, CharacterManager,
    save_character, load_character, list_characters, get_character_info
)
...
class TestCharacterSaveData:
    ...
    def test_from_character(self):
        ...
        with patch('src.adapters.repositories.character_repository.datetime') as mock_datetime:
...
class TestCharacterRepository:
    ...
    def test_save_character_json(self, mock_datetime, mock_file, mock_json_dump):
        ...
        mock_character.strength.value = 16
        mock_character.dexterity.value = 14
        mock_character.constitution.value = 15
        mock_character.intelligence.value = 12
        mock_character.wisdom.value = 13
        mock_character.charisma.value = 10
    ...
    def test_save_character_exception(self):
        ...
        with patch('src.adapters.repositories.character_repository.CharacterSaveData.from_character',
    ...
    def test_load_character_json(self, mock_exists, mock_file, mock_json_load):
        ...
        with patch('src.adapters.repositories.character_repository.CharacterSaveData.to_character') as mock_to_char:
            mock_character = Mock(spec=Character)
    ...
    def test_load_character_exception(self):
        ...
        with patch('src.adapters.repositories.character_repository.Path.exists', return_value=True):
    ...
    def test_delete_character_exception(self):
        ...
        with patch('src.adapters.repositories.character_repository.Path.exists', return_value=True):
            with patch('src.adapters.repositories.character_repository.Path.unlink', 
                      side_effect=Exception("Тестовая ошибка")):
                result = self.repository.delete_character("test.json")
                assert result is False
...
class TestCharacterManager:
    ...
    def test_create_new_character(self, mock_factory):
        ...
        assert result == mock_character
        mock_factory.create_standard_character.assert_called_once_with("Тестовый персонаж")
...
class TestCharacterRepositoryIntegration:
    ...
    def test_full_save_load_cycle(self, mock_class_factory, mock_race_factory):
        ...
        mock_character = Mock(spec=Character)
        mock_character.name = "Интеграционный тест"
        mock_character.level = 3
        mock_character.race.name = "Человек"
        mock_character.character_class.name = "Воин"
        mock_character.strength.value = 14
        mock_character.dexterity.value = 12
        mock_character.constitution.value = 13
        mock_character.intelligence.value = 10
        mock_character.wisdom.value = 11
        mock_character.charisma.value = 9
