...
def test_adventure_logic():
    ...
    for adv in all_adventures:
        print(
            f"  - {adv.name} (файл: {adv.file_name}) - уровень: {adv.starting_level} - активно: {adv.is_active}"
        )
    ...
    # 4. Проверяем уровень
    level_info = level_resolver.get_level_info(1)
    print(
        f"\nНачальный уровень: {level_info.level} (бонус мастерства: {level_info.proficiency_bonus})"
    )
    ...
    if non_tutorial:
        ...
        if active_adventure and active_adventure.file_name == "tutorial_adventure.yaml":
            print("Активное приключение: Начало пути (Tutorial)")
            print("Начальный уровень: 1")
        else:
            print(
                f"Начальный уровень: {level_info.level} (бонус мастерства: {level_info.proficiency_bonus})"
            )
