...
class TestRace:
    ...
    def test_get_effective_ability_bonuses_with_subrace(self) -> None:
        """Тест эффективных бонусов с подрасой."""
        race = Race(name="Тест", description="Тест", ability_bonuses={"strength": 1})

        # С наследующей подрасой
        inheriting_subrace = SubRace("Test", "Test", ability_bonuses={"dexterity": 1}, inherit_base_abilities=True)
        bonuses = race.get_effective_ability_bonuses(inheriting_subrace)
        assert bonuses == {"strength": 1, "dexterity": 1}

        # С не наследующей подрасой
        non_inheriting_subrace = SubRace("Test", "Test", ability_bonuses={"dexterity": 2}, inherit_base_abilities=False)
        bonuses = race.get_effective_ability_bonuses(non_inheriting_subrace)
