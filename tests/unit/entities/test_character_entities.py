...
class TestCharacter:
    ...
    def test_size_property_without_race(self) -> None:
        ...
        assert character.size == Size.MEDIUM
    ...
    def test_character_equality(self) -> None:
        ...
        # Character - это dataclass, поэтому равенство основано на полях
        character1 = Character(name="Персонаж")
        character2 = Character(name="Персонаж")
        character3 = Character(name="Другой персонаж")

        # Разные объекты с одинаковыми данными равны (dataclass behavior)
        assert character1 is not character2  # Разные объекты в памяти
        assert character1 == character2      # Но равны по данным
