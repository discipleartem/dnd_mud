...
class TestRenderer:
    ...
    def test_show_error(self, mock_print):
        ...
        mock_print.assert_called_once_with("\033[1;31m❌ ОШИБКА: Тестовая ошибка\033[0m")
...
class TestInputHandler:
    ...
    def test_get_string_with_default_accepted(self, mock_input):
        ...
        result = self.input_handler.get_string("Введите строку: ", "значение по умолчанию")
    ...
    def test_get_string_empty_not_allowed(self, mock_print, mock_input):
        ...
        result = self.input_handler.get_string("Введите строку: ", allow_empty=False)
    ...
    def test_get_int_validation_range(self, mock_print, mock_input):
        ...
        mock_input.side_effect = ["5", "15", "12"]  # 5 < min, 15 > max, 12 в диапазоне
    ...
    def test_get_int_invalid_format(self, mock_print, mock_input):
        ...
        mock_input.side_effect = ["abc", "12.5", "not a number", "15"]
...
class TestUIIntegration:
    ...
    def test_character_creation_flow(self, mock_print, mock_input):
        ...
        assert class_choice == 2
        
        # Проверяем вызовы рендерера
        calls = [str(call) for call in mock_print.call_args_list]
        assert any("Создание персонажа" in call for call in calls)
        assert any("✓ Создан персонаж: Тестовый персонаж, уровень 5" in call for call in calls)
        assert any("ℹ Раса: 1, Класс: 2" in call for call in calls)
...
class TestErrorHandling:
    ...
    def test_multiple_error_recovery(self, mock_print, mock_input):
        ...
        result = test_input_handler.get_int("Введите число (1-10): ", min_value=1, max_value=10)
