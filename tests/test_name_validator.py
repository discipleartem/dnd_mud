...
class TestNameValidator:
    ...
    def test_name_starting_with_hyphen(self):
        ...
        assert not result.is_valid
        assert "не может начинаться или заканчиваться дефисом" in result.error_message
    ...
    def test_name_with_valid_hyphens(self):
        ...
        for name in valid_names_with_hyphens:
            result = self.validator.validate(name)
            assert result.is_valid, f"Имя '{name}' с дефисами должно быть валидным"
    ...
    def test_name_with_mixed_case(self):
        ...
        for name in mixed_case_names:
            result = self.validator.validate(name)
            assert result.is_valid, f"Имя '{name}' в любом регистре должно быть валидным"
    ...
    def test_name_with_whitespace_trimming(self):
        ...
        for name in names_with_whitespace:
            result = self.validator.validate(name)
            assert result.is_valid, f"Имя '{repr(name)}' с пробелами по краям должно быть валидным"
