...
def demonstrate_race_usage():
    ...
    print(f"💪 Бонусы характеристик: {selected_race.ability_bonuses_description}")
    ...
    if selected_race.subraces:
        print(f"\n🔸 Доступные подрасы:")
        for subrace_id, subrace in selected_race.subraces.items():
        ...
        if selected_subrace:
            ...
            total_bonuses = selected_race.get_total_ability_bonuses(selected_subrace_id)
            print(f"💪 Общие бонусы характеристик:")
