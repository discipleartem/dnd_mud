"""Данные для отображения экипировки персонажа (без Colorama)."""
...
def versatile_grip_hint(
    ...
    if not weapon_is_versatile(weapon_id):
        return ""
    if versatile_active_grip(equipped) == "two_handed":
...
def format_versatile_damage_dice(
    ...
    active = (
        "two" if versatile_active_grip(equipped) == "two_handed" else "one"
    )
    return one_dice, two_dice, active


def loaded_ammunition_qty(
...
def equipped_weapon_for_range(equipped: EquippedState) -> str | None:
...
def build_equipped_display_data(
    character: Character,
    language: str = "ru",
    ...
    if isinstance(main_hand, str) and main_hand:
        ...
        grip_hint = versatile_grip_hint(main_hand, equipped, strings)
        if grip_hint:
    ...
    if ammo_weapon:
        ...
        if ammo_id:
            qty = loaded_ammunition_qty(character.inventory, ammo_id)
    ...
    range_weapon = equipped_weapon_for_range(equipped)
    if range_weapon:
...
def get_equipped_display(
    ...
    """См. build_equipped_display_data."""
    return build_equipped_display_data(character, language)
